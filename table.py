...
app.layout = html.Div(children=[
    ...
    html.Div(children=[
        ...
        html.Div(children=[
            ...
            html.H3('MPV selected: ', style={'fontWeigth': 'bold'}),
...
def update_qc_graph(click_data, result, type, genotype, start_date, end_date):
    ...
    # Add traces
    for test in test_type:
        types_graph.add_trace(go.Scatter(x=filtered_df['day'], y=filtered_df[test], mode='lines+markers', name=test, cliponaxis=True,
        line_shape= "spline", showlegend=True, fill='tonexty',
      ))
    ...
    count_df['value'] = filtered_df[str(type)]
    count_df['adequacy ' + type] = filtered_df['adequacy ' + type]
    count = filtered_df[str(type)].count()
    satisfactory_df = count_df[count_df['adequacy ' + type] == {'satisfactory': 'Yes'}]
    satisfactory = satisfactory_df['value'].count()
    not_processed_df = count_df[count_df['adequacy ' + type] == {'satisfactory': 'No', 'processed': 'processed'}]
    not_processed = not_processed_df['value'].count()
    remainder_df = count_df[count_df['adequacy ' + type] == {'satisfactory': 'No', 'processed': 'Not processed'}]
    ...
    for item in results_list:
        count += 1
        data = filtered_df[filtered_df['result ' + type] == item]
        results_graph.add_trace(go.Scatter(x=filtered_df["day"], y= data[type], mode="markers+lines", name=item, marker_symbol= count, showlegend=True))

    results_graph.update_layout(legend_title_text = "Result")
    results_graph.update_yaxes(title_text=type)
    results_graph.update_xaxes(title_text='day')


    filtered_df = filtered_df[filtered_df['result ' + type] == result]
    mvp_graph = go.Figure()
    for gn in genotype_list:
        data = filtered_df[filtered_df['mvp'] == gn]
        mvp_graph.add_trace(go.Scatter(x=filtered_df['day'], y=data[type], mode='markers+lines', name=gn, showlegend=True))
    ...
    for qc_result in choices:
        ...
        size = data['day'].size
        # print(size)
        qc_graph.add_trace(go.Scatter(x=data['day'], y=data[type], marker_symbol= count, mode='markers+lines', name=qc_result, showlegend=True))
        qc_graph.update_layout(legend_title_text = "Combinations")
        qc_graph.update_yaxes(title_text='number')

    return f'{type}', f'Adequacy: {message}. Processed: {processed}.',f'{result}', f'{genotype}', types_graph,  adequacy_graph, results_graph, mvp_graph, qc_graph
